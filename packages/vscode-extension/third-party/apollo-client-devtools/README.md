...
3. create the application plugin by copying the contents of `tab.js` and `hook.js` into `lib/plugins/apollo-client-devtools.js`,
   between the corresponding `BEGIN`/`END` comment markers.
   The file is then `require`d by the application when the apollo client is loaded, via our babel transform.
