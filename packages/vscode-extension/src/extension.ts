...
export async function activate(context: ExtensionContext) {
  ...
  async function showIDEPanel() {
    ...
    if (panelLocation !== "tab") {
      ...
      TabPanel.render(context, getAppRootFolder());
    }
  }
  ...
  context.subscriptions.push(
    window.registerWebviewViewProvider(
      ...
      new SidePanelViewProvider(context, getAppRootFolder()),
  ...
  context.subscriptions.push(
    workspace.onDidChangeConfiguration(async (event: ConfigurationChangeEvent) => {
      ...
      if (event.affectsConfiguration("launch")) {
        ...
        const oldAppRoot = getAppRootFolder().getAppRoot();
        if (config.appRoot === oldAppRoot) {
          return;
        }
        const appRoot = await setupAppRoot();
