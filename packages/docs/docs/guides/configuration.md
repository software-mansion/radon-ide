...
| **usePrebuild**                   | Boolean that when set to `true` makes Radon IDE run `expo prebuild` command before building the app and when set to `false` Radon IDE will always skip it. By default, Radon uses Prebuild for projects with `expo dev-client` and skips it for other projects. If your `android` or `ios` directory is checked into git, Radon treats it as source and skips Prebuild so that your files are not overwritten. The IDE panel then offers to run Prebuild and setting this option to `true` always runs it. |
