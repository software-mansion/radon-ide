...
| **usePrebuild**                   | Whether Radon runs `expo prebuild` before building. `true` always runs it, `false` never does. When unset, Radon runs it for `expo-dev-client` projects, unless the `android` or `ios` directory is checked into git; then it skips Prebuild so your files are not overwritten and the panel offers to run it anyway or keep ignoring it; either choice is saved here as `true` or `false`.                                     |
