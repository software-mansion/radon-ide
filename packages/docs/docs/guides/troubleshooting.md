...
Radon IDE uses node version inherited from a shell run in the application root and checks if it satisfies `react-native` minimum requirements.
If you have a specific setup that uses node version from outside this range or you want to narrow down the scope of compatible versions for your internal needs, you can do so by specifying `engines` field of your application's `package.json`.
If you have a problem installing a compatible node version, we recommend using a tool like [nvm](https://github.com/nvm-sh/nvm).
