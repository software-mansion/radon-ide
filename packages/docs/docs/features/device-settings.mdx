...
1. **Device Appearance** - change between the light and dark mode.
2. **Text Size** - a slider that allows to change the font size on the device.
3. **Press Home Button** - simulate a press of the home button.
4. **Open App Switcher** - open the app switcher to quickly switch between opened apps.
5. **Rotate Device** - rotate the device or set its orientation.
...
7. **Send File** - send a file from your machine to the device.

File sending also supports **Drag and Drop** - you can drag the file from the file explorer and drop it onto the device preview holding `shift` key.

8. **Location** - enable location services and set static device geolocation using coordinates.
...
9. **Localization** - set the device [localization](https://developer.mozilla.org/en-US/docs/Glossary/Localization). Changing the localization setting reboots the device. This option isn't available on Android.

<img width="450" src="/img/docs/ide_localization.png" className="shadow-image" />

10. **Volume** - adjust the device volume using the minus and plus controls.

11. **Action Button** - simulate the press of device action button.

12. **Reset Permissions** - reset the device privacy permissions.
...
13. **Open Deep Link** - open deep links and website URLs on the device. Website URLs open in the default device browser.

<img width="450" src="/img/docs/ide_deep_link.png" className="shadow-image" />

14. **Enable Replays** - turn the "replay" functionality on or off. When enabled, a `Replay` button appears in the Radon IDE panel.

<img width="300" src="/img/docs/ide_replays_enabled.png" className="shadow-image" />

15. **Show Touches** - toggle whether touch indicators are visible on the device preview.

16. **Clipboard Sync** - toggle automatic clipboard synchronization between your computer and the connected device. When enabled, copying text on either side makes it available for pasting on the other.

17. **Show Device Frame** - toggle the device frame (bezels) in the preview on or off.
