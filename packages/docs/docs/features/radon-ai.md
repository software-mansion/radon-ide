...
To enable it, go to the `Settings -> VS Code Settings` tab (or press `Cmd + ,`), type `Radon AI` into the search bar, press the first result and select `Enabled`.
:::
...
Within editor settings, type "Radon AI" into the search bar, press the first result and select `Disabled`.
