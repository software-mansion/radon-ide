...
describe("14 - Error tests", () => {
  ...
  itIf(
    ...
    async function () {
      ...
      await appManipulationService.waitForAppToLoad();
      const originalText = exec(
        `cat ./data/react-native-app/shared/automatedTests.tsx`
      ).stdout;
      try {
        exec(
          "echo \"import notExisting from 'not-existing'; notExisting();\" > ./data/react-native-app/shared/automatedTests.tsx"
        );
        await driver.wait(
          async () => {
            try {
        ...
        assert.equal(await dialogTitle.getText(), "Bundle error");
        exec(
          `echo "${originalText}" > ./data/react-native-app/shared/automatedTests.tsx`
        );
      } finally {
      }
    }
